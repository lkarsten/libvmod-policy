...
from VPOLServer import BaseVPOLRequestHandler, ClientError
...
class SORBScheck(BaseVPOLRequestHandler):
    ...
    def policy(self):
        if "client_identity" not in self.meta:
            raise ClientError("No client_identity found")

        _ip = self.meta["client_identity"].split(".", 4)
        if len(_ip) != 4:
